...
class TargetEnvironment:
    ...
    target_cpu_profile_id: str = "x86_64.generic.v1"
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.target_cpu_profile_id, str) or not self.target_cpu_profile_id:
            raise TypeError("TargetEnvironment.target_cpu_profile_id must be a non-empty string")
    ...
    def fixed_sysv_amd64_gnu_att(
        ...
        *,
        ...
        return cls(
            ...
            target_cpu_profile_id=target_cpu_profile_id,
        )
...
def _derive_privileged_functional_0(
    *, source_model, candidate_plan, target_environment,
    ...
    expected_preserved = tuple(candidate_plan.metadata.get(
        "preserved_environment_semantics", ()
    ))
    expected_not_preserved = tuple(candidate_plan.metadata.get(
        "not_preserved_environment_semantics", ()
    ))
    if (
        expected_preserved
        and contract.preserved_environment_semantics != expected_preserved
    ) or (
        expected_not_preserved
        and contract.not_preserved_environment_semantics != expected_not_preserved
    ):
        return TargetConstraintDerivationResult.failure(
            plan_id=candidate_plan.plan_id,
            reason_codes=(
            ...
            details={"reason": "functional-environment-preservation-boundary-mismatch"},
        )
    state = source.state
    if (
        contract.source_execution_profile != state.execution_profile.value
