...
def validate_common(request: SemanticProofRequest):
    ...
    operation_complete=s.operation.complete or (p.kind is TargetLoweringKind.VIRTUAL_PRIVATE_FRAME and s.stack_frame is not None and s.stack_frame.virtual_private_frame_eligible) or (p.kind is TargetLoweringKind.ABI_WRAPPER_CALL and s.abi_effects is not None and s.abi_effects.complete)
...
def run_semantic_proof_gate(*, source_model, preservation_decision=None, candidate_plan, constraints, target_environment, target_semantic_catalog=None, compiler_capabilities=None, helper_contract_registry=None):
    ...
    from . import phase6d_stack_rebinding as sr, phase6d_virtual_private_frame as vp, phase6d_abi_wrapper as aw
