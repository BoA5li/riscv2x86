...
[{"directory":"$ROOT/$SRC","command":"clang-10 --target=riscv64 -march=rv64gc -c sample.c","file":"sample.c"}]
EOF
...
# Step 2: Python 翻译 + 校验
PYTHONPATH=pytools python3 -m riscv2x86_py.cli \
    --in "$OUT/report.json" --out "$OUT/translated.json" --xlen 64
...
clang-10 --target=x86_64-pc-linux-gnu -O2 -c "$OUT/sample.c" -o "$OUT/sample.o"
echo "=== build OK ==="
